...
void decode_for(loop_control_type* l_control, parse_struct2* parse_in, UINT loop_start2) {
	...
	if (l_control->depth_out > 0) {
		...
		while (parse_in->line[loop_start2].line[ptr] == ' ' || parse_in->line[loop_start2].line[ptr] == '\t')	ptr++;
		if (parse_in->line[loop_start2].line[ptr] == 'f' && parse_in->line[loop_start2].line[ptr + 1] == 'o' && parse_in->line[loop_start2].line[ptr + 2] == 'r')
			ptr += 3;
		else
			debug++;
		while (parse_in->line[loop_start2].line[ptr] == ' ' || parse_in->line[loop_start2].line[ptr] == '\t' || parse_in->line[loop_start2].line[ptr] == '(')	ptr++;
		getWord(parse_in, loop_start2, &ptr);
		if (strcmp(parse_in->word, "int") == 0 || strcmp(parse_in->word, "UINT") == 0 || strcmp(parse_in->word, "UINT8") == 0 || strcmp(parse_in->word, "UINT16") == 0) {
			strcpy_word(l_control->index[l_control->depth_out].unit_type, parse_in);
		}
		else {
			debug++;
		}
		...
		if (parse_in->word[0] != '\0')
			strcpy_word(l_control->index[l_control->depth_out].name, parse_in);
		else
			debug++;
		while (parse_in->line[loop_start2].line[ptr] == ' ' || parse_in->line[loop_start2].line[ptr] == '\t')	ptr++;
		if (parse_in->line[loop_start2].line[ptr] == '=') {
			ptr++;
			if (parse_in->line[loop_start2].line[ptr] == '=') {
				debug++;
			}
		...
		if (get_integer(&number, parse_in->word)) {
			l_control->index[l_control->depth_out].initial = number;
		}
		else {
			debug++;
		}
		...
		while (parse_in->line[loop_start2].line[ptr] == ' ' || parse_in->line[loop_start2].line[ptr] == '\t')	ptr++;
		getWord(parse_in, loop_start2, &ptr);
		if (strcmp(l_control->index[l_control->depth_out].name, parse_in->word) != 0) {
			debug++;
		}
		while (parse_in->line[loop_start2].line[ptr] == ' ' || parse_in->line[loop_start2].line[ptr] == '\t')	ptr++;
		if (parse_in->line[loop_start2].line[ptr] == '<') {
		...
		getWord(parse_in, loop_start2, &ptr);
		if (get_integer(&number, parse_in->word)) {
			l_control->index[l_control->depth_out].limit = number;
		}
		else {
			debug++;
		}
		...
		while (parse_in->line[loop_start2].line[ptr] == ' ' || parse_in->line[loop_start2].line[ptr] == '\t')	ptr++;
		if (parse_in->line[loop_start2].line[ptr] == '+' && parse_in->line[loop_start2].line[ptr + 1] == '+') {
			l_control->index[l_control->depth_out].increment = 1;
			ptr += 2;
		}
		else if (parse_in->line[loop_start2].line[ptr] == '+' && parse_in->line[loop_start2].line[ptr + 1] == '=') {
			ptr += 2;
			while (parse_in->line[loop_start2].line[ptr] == ' ' || parse_in->line[loop_start2].line[ptr] == '\t')	ptr++;
			getWord(parse_in, loop_start2, &ptr);
			if (get_integer(&number, parse_in->word)) {
				l_control->index[l_control->depth_out].increment = number;
			}
			else {
				debug++;
			}
...
void add_candidate2(candidate_set_type2* candidate, int range_low, int range_high, int skip) {
...
void parse_candidate2(candidate_set_type2* candidate, parse_struct2* parse_temp, loop_control_type* l_control, array_list_struct* array_list, char in_out) {
	UINT debug = 0;
				...
				case ')':
					if (parse_temp->line[parse_temp->index].line[parse_temp->ptr + 1] == ']') {
						...
						if (candidate->entry[candidate->count].range_high - candidate->entry[candidate->count].range_low > 0x800 || force_candidate)
							add_candidate2(candidate, candidate->entry[candidate->count].range_low, candidate->entry[candidate->count].range_high, 0);
					}
					strcat_p2(&candidate->entry[candidate->count].base, (char*)")");
					...
					if (parse_temp->line[parse_temp->index].line[parse_temp->ptr + 1] == '<') {
						...
						if (get_integer(&number, parse_temp->word)) {
							...
							if (number > 0x0800 || (range_high - range_low) > 0x0800 || force_candidate)
								add_candidate2(candidate, range_low, range_high, skip);
						}
						else {
							debug++;
						}
										...
										if (parse_temp->word[0]!='\0') {// need to chack for "for", "if", or a function call "func("
											if (VariableTypeMatch(&match_index, parse_temp->word, var_type)) {
												...
												if (parse_temp->index >= 104)
													debug++;
												if (parse_temp->line[parse_temp->index].line[parse_temp->ptr] == '[') {
													parse_candidate2(&candidate, parse_temp, &l_control, &array_list, 2);
												...
												while (parse_temp->line[parse_temp->index].line[parse_temp->ptr] != '\0') {
													if (parse_temp->line[parse_temp->index].line[parse_temp->ptr] == '/' && parse_temp->line[parse_temp->index].line[parse_temp->ptr + 1] == '/') {// detect comment - skip
														...
														if (parse_temp->word[0]!='\0') {
															if (parse_temp->line[parse_temp->index].line[parse_temp->ptr] == '[') {
																parse_candidate2(&candidate, parse_temp, &l_control, &array_list, 1);
															}
															else {
															}
														}
